...
child run에는 `method: cognitive_transition_v3`인 불변 artifact가 하나 저장된다.
...
- `GET /v1/deliberations/{id}/transition`
  - 정상: sanitized `cognitive_transition_v3` payload, HTTP `200`
...
- **질문↔evidence 의미 관련성은 기본 경로에서 검증하지 않는다.** 단일 모델·
  결정적 규칙만으로 “이 증거가 이 질문에 답하는가”를 독립 판정할 수 없으며,
  날조하지 않는다. 따라서 child가 인용한 무관 evidence로도 `evidence:no_evidence`
  claim이 verified 될 수 있다.
- 의미 관련성은 **옵션 2차 모델 게이트**로 완화 가능: `critic2` provider가
  설정되면 transition *생성* 시 `semantic_relevance_v1`이 교집합 evidence
  본문으로 “KR 질문에 실제로 답하는가”를 1회 판정하고, 결과를 payload의
  `semantic_verdicts`에 **동결(freeze)** 한다 (provider/model provenance·
  input digest 포함, 원문 금지). 질문 길이 상한·프롬프트에 실제 삽입된
  evidence id만 `verification_evidence_ids`에 기록하며, evidence 0건 삽입·
  본문 조회 실패·provider 예외·factory 예외·비JSON·`related=false`는 모두
  보수적으로 `claimed_unverified`(승격 금지). **replay는 frozen
  `semantic_verdicts`만으로 LLM 없이 재계산**한다 (live provider 호출 금지).
  **진짜 독립 판정은 `claude-cli`일 때만**이며, `codex`/`stub`는
  self-consistency 수준이다. 기본은 off(`semantic_verdicts={}`).
...
- self-digest가 맞고 impact method가 `citation_scope_projection_v2`가 아니면,
  transition method가 `cognitive_transition_v3`가 아니면 재계산을 건너뛰고
  거짓 mismatch를 내지 않는다. 레거시 `cognitive_transition_v2`는
  method_version_mismatch로 skip한다.
...
- `cognitive_transition_v3`가 claimed/verified/unverified와 critic-promoted gap,
  동결 `semantic_verdicts`를 저장·조회한다.
