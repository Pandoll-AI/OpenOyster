...
- 의미 관련성은 **옵션 2차 모델 게이트**로 완화 가능: `critic2` provider가
  설정되면 transition 시 `semantic_relevance_v1`이 교집합 evidence 본문으로
  “KR 질문에 실제로 답하는가”를 판정한다. provider 예외·비JSON·`related=false`
  는 보수적으로 `claimed_unverified`(승격 금지). **진짜 독립 판정은
  `claude-cli`일 때만**이며, `codex`/`stub`는 self-consistency 수준이다.
  기본은 off(무변화).
