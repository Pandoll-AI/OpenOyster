...
def _output_contract_block(stage: str) -> str:
    ...
    return (
        ...
        "Optional flip_conditions[].predicate has query_terms (1..8 non-empty strings, each "
        "<=100 chars) used for deterministic Pack-evidence monitoring; omit predicate when "
        "the flip condition should remain dossier-only and not watched.\n"
        ...
        "substring of that snapshot's prompt_visible_payload.text and at least 12 non-padding "
        "characters after strip (MIN_QUOTE_CHARS). For json_pointer anchors omit "
